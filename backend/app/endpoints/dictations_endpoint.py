...
def create_dictation(
    dictation_in: DictationCreate, session: Session = Depends(get_session)
):
    ...
    new_dictation = Dictation(
        title=dictation_in.title, content_reference=dictation_in.content_reference
    )
...
@router.get(
    "/{dictation_id}", response_model=DictationResponse, status_code=status.HTTP_200_OK
)
